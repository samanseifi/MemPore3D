...
def solve_phi(phi: np.ndarray, Vm: np.ndarray, k_mem: int, V_applied: float, dx2: float, dy2: float, dz2: float, tol: float, max_iter: int) -> Tuple[np.ndarray, int]:
    """
    Solves the 3D Laplace equation for the electric potential `phi` via Jacobi iteration.
    """
    inv_lap_den = 1.0 / (2.0/dx2 + 2.0/dy2 + 2.0/dz2)
    iters_done = max_iter

    for i in range(max_iter):
        phi_old = phi.copy()

        # Jacobi update for interior nodes
        phi[1:-1, 1:-1, 1:-1] = (
        ...
        # --- Boundary Conditions (re-applied each iteration) ---
        # Dirichlet BCs on top/bottom
        phi[:, :, 0] = -V_applied / 2.0
        phi[:, :, -1] = V_applied / 2.0
        # Internal "jump" BC at the membrane plane set by Vm
        phi[:, :, k_mem - 1] = -Vm / 2.0
        phi[:, :, k_mem + 1] = Vm / 2.0
        # phi[:,:, k_mem] = phi[:,:, k_mem + 1] - phi[:,:, k_mem - 1] 
        # Neumann BCs on side walls (zero flux)
        phi[0, :, :] = phi[1, :, :]
        phi[-1, :, :] = phi[-2, :, :]
        phi[:, 0, :] = phi[:, 1, :]
        phi[:, -1, :] = phi[:, -2, :]
...
def simulate_membrane_charging(
    ...
    for n in range(nsteps):
        ...
        if n % save_interval == 0 or n == nsteps - 1:
            ...
            print(f"Time: {t*1e9:8.2f} ns [{n+1:>{len(str(nsteps))}}/{nsteps}] | Avg Vm: {avg_Vm:.4f} V | Solver Iters: {iters}")
    ...
    np.savez_compressed(
        "membrane_charging_results_jacobian.npz",
