...
class PhaseFieldSolver:
    ...
    def update_psi(self, psi: np.ndarray, Vm: np.ndarray, dt: float, delta_C: float) -> np.ndarray:
        """
        Advances the phase-field `psi` by one time step, including the
        electroporative force from Vm.
        """
        # --- Chemical potential from line/surface tension (original terms) ---
        mu_chem = (self.dyn.gamma / self.Cg) * (self.gp(psi) / self.dyn.transition_thickness) + self.dyn.sigma * self.dH(psi)

        # --- Chemical potential from electrostatic energy ---
        # This is the new electroporative force term. It is negative because a
        # positive force should decrease psi (open the pore).
        mu_elec = 0.0 # -0.5 * delta_C * self.dH(psi) * Vm**2

        # --- Combine terms and perform time step in Fourier space ---
        rhs = -self.dyn.M * (mu_chem + mu_elec)
...
def simulate_electroporation(
    ...
    if dyn.transition_thickness is None:
        dyn.transition_thickness = 2.0 * dx
    ...
    for n in range(nsteps):
        ...
        H = smooth_step(psi)
        G_m_map, _, C_eff_map = blend_properties(H, props, dom, elec)
        ...
        # --- Update Phase Field (psi) ---
        psi = psi_solver.update_psi(psi, Vm, dt, delta_C)
